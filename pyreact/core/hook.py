...
class HookContext:
    ...
    def use_reducer(self, reducer, initial, *, init_fn=None, deps=None):
        ...
        if idx >= len(self.hooks):  # first mount
            ...
            if deps_key is not None and old_deps != deps_key:             # Reinit due to deps change (optional)

                state = init_fn(initial) if init_fn is not None else initial
                self.hooks[idx] = (state, reducer, deps_key)
            else:
                if old_reducer is not reducer or old_deps != deps_key: # Update reducer reference (dispatch uses the current reducer)
                    self.hooks[idx] = (state, reducer, deps_key if deps is not None else old_deps)
    ...
    def unmount(self):
        # 1. clean-ups gerados por use_effect APENAS nos índices rastreados
        for idx in list(getattr(self, "_effect_slots", set())):
            if idx < len(self.hooks):
                self._run_cleanup_slot(self.hooks[idx])

        # 2. remover subscrições de Context
        ...
        # 3. desmontar filhos recursivamente
